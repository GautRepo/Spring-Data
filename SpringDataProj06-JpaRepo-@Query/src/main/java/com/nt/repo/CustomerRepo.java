...
public interface CustomerRepo extends JpaRepository<Customer, Integer>{
	...
	Iterable<Customer> getAllCustomerByCadd(String city);
	
	
	@Query("FROM Customer WHERE billAmt>=?1 AND billAmt<=?2")
	Iterable<Customer> getAllCustomerByCities(double a, double b);
	
	/*_____________________________________________________________*/
	
	//@Query("FROM Customer WHERE cadd IN (:a,:b,:c)")
	@Query("FROM Customer WHERE cadd IN (:a,:b,:c)")
	Iterable<Customer> getAllCustomerByCities(@Param("a")String city1,@Param("b") String city2,@Param("c") String city3);

	/*_______Select bulk operation with named parameter_________*/

	@Query("Select cno, cname From Customer where cadd=:cty")
	Iterable<Object[]> getAllCustomerByCity(@Param("cty") String city);
	...
	@Query("Select cname,cno from Customer Where cname=:name")
	List<Object[]> getDateValueByCname(String name);
	
	
	/*____________bulk update ___________________*/
	
	@Modifying
	@Query("Update Customer set billAmt=billAmt+:extraAmt where cadd=:city")
	int modifyCustBillAmtByCity(String city, double extraAmt);
	
	/*_____________Delete Customer____________________*/
	
	@Modifying
	@Query("Delete from Customer where cadd=:city")
	int deleteCustByCity(String city);
	
	/*_______________________________________________________________ Notes__________________________________________*/
